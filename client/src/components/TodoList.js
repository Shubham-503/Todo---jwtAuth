  ...
  return (
    <div className='todoList flex flex-wrap  justify-around m-4 p-4 w-full'>
      ...
      {todos && todos.map(todo => {
        // setTitle(todo.title)
        return <div className="antialiased   text-slate-700  md:w-1/2 w-full" key={uuidv4()}>
          <div className="max-w-lg mx-4 my-10 bg-white p-8 rounded-xl shadow shadow-slate-300">
            <div className="flex flex-row justify-between items-center">
              <div>
                <h1 className="text-3xl font-medium">{todo.title}</h1>
              </div>
              <div className="inline-flex space-x-2 items-center ml-12">
                ...
                <button href="#" className="p-2 border border-red-200 rounded-md inline-flex space-x-1 items-center bg-red-600 hover:bg-red-500"
            ...
            <p className="text-xs text-slate-500 text-center">{todo.updatedAt}</p>
          </div>
